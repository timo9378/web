...
import { Sparkles, Copy, Check, Send, FileText, RotateCcw, RotateCw } from 'lucide-react';
...
async function fetchLLM(
  messages: LLMMessage[],
  { maxTokens = 4096, temperature = 0.75 }: { maxTokens?: number; temperature?: number } = {},
): Promise<string> {
  ...
  try {
    ...
    console.log('[ArticleGen] fetchLLM:', {
      maxTokens,
      temperature,
...
async function generateLongForm(
  ...
  const outlineMessages: LLMMessage[] = [{ role: 'system', content: outlineSystem }];
  if (guide.trim()) {
  ...
  try {
    ...
    if (match) {
      try {
        outline = JSON.parse(match[1]) as Outline;
      } catch {
        /* fallback */
      }
    }
  ...
  // ── Phase 2：逐段展開 ────────────────────────────────
  const outlineSummary = sections.map((s, i) => `${i + 1}. ${s.heading ?? '（開場段）'}：${s.brief ?? ''}`).join('\n');
  ...
  for (let idx = 0; idx < sections.length; idx++) {
    ...
    if (isFirst) {
      expandSystem +=
        `- 這是文章開頭，用自然的第一人稱開場，你的標題是「${heading}」，請務必以 \`## ${heading}\` 作為第一行\n` +
        '- 寫完開場段後開始展開第一個主題\n';
    } else {
    ...
    expandSystem +=
      '- 篇幅充實但不注水(嚴禁腦補不存在的事件或細節)，這段應寫 600-1000 字\n' +
