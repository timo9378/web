...
import {
  ...
  adminBlacklistQueryOptions,
  adminKeywordFiltersQueryOptions,
} from '@/data/adminData';
...
export default function CommentsManager() {
  ...
  // Dialogs
  const [replyDialog, setReplyDialog] = useState<{ open: boolean; comment: Comment | null }>({
    open: false,
    comment: null,
  });
  const [replyText, setReplyText] = useState('');
  const [editDialog, setEditDialog] = useState<{ open: boolean; comment: Comment | null }>({
    open: false,
    comment: null,
  });
  ...
  const invalidateKeywords = () =>
    queryClient.invalidateQueries({ queryKey: adminKeywordFiltersQueryOptions.queryKey });
  ...
  const blockIp = async (ip: string) => {
    try {
      const res = await fetch('/api/admin/blacklist', {
        method: 'POST',
        headers: authHeaders(),
        body: JSON.stringify({ ip, reason: '來自留言管理封鎖' }),
      });
      if (res.ok) {
        toast.success(`已封鎖 IP: ${ip}`);
      }
    } catch {
      toast.error('封鎖失敗');
    }
  ...
  const addBlacklist = async () => {
    ...
    try {
      const res = await fetch('/api/admin/blacklist', {
        method: 'POST',
        headers: authHeaders(),
        body: JSON.stringify({ ip: newBlacklistIp, reason: newBlacklistReason }),
      });
      if (res.ok) {
        toast.success('已加入黑名單');
        setNewBlacklistIp('');
        setNewBlacklistReason('');
        void invalidateBlacklist();
      }
    } catch {
      toast.error('操作失敗');
    }
  ...
  const addKeyword = async () => {
    ...
    try {
      const res = await fetch('/api/admin/keyword-filters', {
        method: 'POST',
        headers: authHeaders(),
        body: JSON.stringify({ keyword: newKeyword, action: newKeywordAction }),
      });
      if (res.ok) {
        toast.success('已新增過濾詞');
        setNewKeyword('');
        void invalidateKeywords();
      }
    } catch {
      toast.error('操作失敗');
    }
  ...
  const formatDate = (d: string) =>
    new Date(d).toLocaleString('zh-TW', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
    ...
    <div className="p-6 space-y-6">
        ...
        <>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button
              ...
              className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${
                statusFilter === 'all'
                  ? 'border-foreground/20 text-foreground bg-accent/50'
                  : 'border-border/40 text-muted-foreground hover:text-foreground/70'
              }`}
            >
            ...
            <div className="space-y-2">
              {comments.map((c) => {
                ...
                return (
                  <div
                    key={c.id}
                    className="group rounded-lg border border-border/40 bg-card/50 hover:bg-accent/20 transition-colors"
                  >
                    <div className="p-4">
                      {/* Top row: author + meta */}
          ...
          {total > 50 && (
            <div className="flex justify-center gap-2 pt-4">
              ...
              <Button variant="outline" size="sm" disabled={comments.length < 50} onClick={() => setPage((p) => p + 1)}>
                下一頁
              </Button>
            </div>
          )}
      ...
      <Dialog open={replyDialog.open} onOpenChange={(o) => setReplyDialog({ open: o, comment: replyDialog.comment })}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setReplyDialog({ open: false, comment: null })}>
              取消
            </Button>
            <Button
              onClick={() => {
      ...
      <Dialog open={editDialog.open} onOpenChange={(o) => setEditDialog({ open: o, comment: editDialog.comment })}>
        <DialogContent className="sm:max-w-md">
          ...
          />
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditDialog({ open: false, comment: null })}>
              取消
            </Button>
            <Button
              onClick={() => {
