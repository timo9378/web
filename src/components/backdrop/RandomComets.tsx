...
const RandomComets = ({ count = 1 }: RandomCometsProps) => {
  // Reduce default comet count to 1
  const { isVisible } = usePageVisibility();
  const [comets, setComets] = useState<Comet[]>(() => Array.from({ length: count }, createComet));

  // Remove useEffect with setInterval

  const handleAnimationComplete = (id: number) => {
    setComets((prevComets) => prevComets.map((comet) => (comet.id === id ? createComet() : comet)));
  };
      ...
      <AnimatePresence>
        {comets.map((comet) => (
          <motion.div
            ...
            onAnimationComplete={() => {
              handleAnimationComplete(comet.id);
