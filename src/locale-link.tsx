...
import { useLocale, localizedPath } from './lib/useLocale';

/**
 * 內部導覽連結:自動帶上「目前 locale」前綴,讓使用者在 /en 下點連結還是留在 /en/*。
 * to 用無前綴邏輯路徑(如 '/'、'/about'、'/blog/39')。
 *
 * 本檔刻意只 export 這一個元件:useLocale / useLocaleNavigate / localizedPath
 * 已搬到 lib/useLocale.ts,元件與非元件混在同一模組會讓 Fast Refresh 退回整頁重載。
 */
