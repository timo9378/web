...
/// Howard Hinnant 的 `civil_from_days`：距 1970-01-01 的天數 → (年, 月, 日)。
///
/// 這份原本在 util.rs 與 handlers/thirdparty.rs 各有一份**逐字相同**的拷貝。
...
pub fn iso_from_millis(ms: i64) -> String {
    ...
    let (year, m, d) = civil_from_days(ms.div_euclid(86_400_000));
